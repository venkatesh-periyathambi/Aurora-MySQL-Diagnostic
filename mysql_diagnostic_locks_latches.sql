-- ============================================================================
-- MySQL 8.4 Diagnostic Script: Locks, Latches & Performance Schema
-- Use Case: Query works individually but fails/blocks when run in parallel
-- ============================================================================
-- Run as a user with SELECT on performance_schema and PROCESS privilege.
-- Some queries require SUPER or SYSTEM_VARIABLES_ADMIN.
-- ============================================================================

-- ============================================================================
-- SECTION 1: CURRENT SESSION & SERVER STATE
-- ============================================================================

SELECT '=== SERVER VERSION & UPTIME ===' AS section;
SELECT VERSION() AS mysql_version, @@hostname AS hostname, NOW() AS current_time;
...
SELECT '=== INNODB ENGINE STATUS (LOCK SECTION) ===' AS section;
...
SELECT @@transaction_isolation AS global_isolation_level;
SELECT @@tx_isolation AS session_isolation_level;
...
SELECT '=== PERFORMANCE SCHEMA MUTEX INSTANCES (Top Contended) ===' AS section;
SELECT
    NAME AS mutex_name,
    OBJECT_INSTANCE_BEGIN,
    LOCKED_BY_THREAD_ID
...
SELECT '=== LATEST DETECTED DEADLOCK (from InnoDB status) ===' AS section;
-- The deadlock info is in SHOW ENGINE INNODB STATUS output above.
-- Additional check via performance_schema:

SELECT '=== RECENT DEADLOCK COUNT ===' AS section;
SHOW GLOBAL STATUS LIKE 'Innodb_deadlocks';

SELECT '=== LOCK WAIT TIMEOUT ERRORS ===' AS section;
...
SELECT '=== STATEMENTS WITH HIGHEST LATENCY VARIANCE ===' AS section;
SELECT
    DIGEST_TEXT,
    COUNT_STAR AS exec_count,
...
-- ============================================================================
-- SECTION 9: INNODB BUFFER POOL & ADAPTIVE HASH INDEX
-- ============================================================================
...
SELECT '=== ADAPTIVE HASH INDEX STATUS ===' AS section;
SHOW GLOBAL STATUS LIKE 'Innodb_adaptive_hash%';

SELECT '=== INNODB METRICS (Lock & Latch related) ===' AS section;
SELECT
    NAME,
    SUBSYSTEM,
    COUNT,
...
WHERE SUBSYSTEM IN ('lock', 'latch', 'transaction', 'adaptive_hash_index')
    AND COUNT > 0
ORDER BY SUBSYSTEM, COUNT DESC;

-- ============================================================================
-- SECTION 10: GAP LOCKS & NEXT-KEY LOCKS (Critical for parallel issues)
-- ============================================================================
...
-- ============================================================================
-- SECTION 11: AUTO-INCREMENT LOCKS (Common parallel insert issue)
-- ============================================================================

SELECT '=== AUTO-INCREMENT LOCK MODE ===' AS section;
SELECT @@innodb_autoinc_lock_mode AS autoinc_lock_mode;
-- 0 = traditional (table-level lock), 1 = consecutive, 2 = interleaved (best for parallel)
...
-- ============================================================================
-- SECTION 12: INNODB CONFIGURATION (Relevant to contention)
-- ============================================================================

SELECT '=== INNODB LOCK/CONCURRENCY CONFIGURATION ===' AS section;
SELECT
    ...
    @@innodb_spin_wait_pause_multiplier AS spin_wait_pause_multiplier,
    ...
    @@innodb_page_cleaners AS page_cleaners,
    @@innodb_parallel_read_threads AS parallel_read_threads,
    @@innodb_flush_log_at_trx_commit AS flush_log_at_trx_commit,
    @@innodb_log_buffer_size AS log_buffer_size,
    @@innodb_change_buffering AS change_buffering,
    @@innodb_io_capacity AS io_capacity,
    @@innodb_io_capacity_max AS io_capacity_max;

SELECT '=== CONNECTION & THREAD POOL SETTINGS ===' AS section;
SELECT
    ...
    @@thread_handling AS thread_handling;
...
-- ============================================================================
-- SECTION 13: REDO LOG & UNDO LOG CONTENTION
-- ============================================================================

SELECT '=== REDO LOG STATUS ===' AS section;
SHOW GLOBAL STATUS LIKE 'Innodb_log%';
SHOW GLOBAL STATUS LIKE 'Innodb_os_log%';

SELECT '=== REDO LOG WAITS (Indicates log contention) ===' AS section;
SELECT
    EVENT_NAME,
    COUNT_STAR,
    SUM_TIMER_WAIT / 1000000000 AS total_wait_ms,
...
SELECT '=== UNDO LOG / HISTORY LIST ===' AS section;
SHOW GLOBAL STATUS LIKE 'Innodb_history_list_length';
SHOW GLOBAL STATUS LIKE 'Innodb_purge%';

-- ============================================================================
-- SECTION 14: MEMORY & TEMP TABLE CONTENTION
-- ============================================================================

SELECT '=== MEMORY ALLOCATION EVENTS ===' AS section;
SELECT
    EVENT_NAME,
    CURRENT_COUNT_USED,
...
-- ============================================================================
-- SECTION 15: FILE I/O CONTENTION
-- ============================================================================

SELECT '=== FILE I/O BY EVENT (Top Contention) ===' AS section;
SELECT
    EVENT_NAME,
...
-- ============================================================================
-- SECTION 16: PREPARED STATEMENTS & STATEMENT HISTORY
-- ============================================================================
...
-- ============================================================================
-- SECTION 17: TRANSACTION HISTORY (Recent completed transactions)
-- ============================================================================

SELECT '=== RECENT TRANSACTION EVENTS ===' AS section;
SELECT
    THREAD_ID,
    EVENT_NAME,
    STATE,
...
SELECT '=== TRANSACTION SUMMARY BY THREAD (Active users) ===' AS section;
SELECT
    ets.THREAD_ID,
    t.PROCESSLIST_USER,
    t.PROCESSLIST_DB,
...
-- ============================================================================
-- SECTION 18: ERROR & WARNING SUMMARY
-- ============================================================================
...
-- ============================================================================
-- SECTION 19: SYS SCHEMA VIEWS (Convenience queries)
-- ============================================================================

SELECT '=== sys.innodb_lock_waits (Formatted view) ===' AS section;
...
-- ============================================================================
-- SECTION 20: PERFORMANCE SCHEMA INSTRUMENTATION CHECK
-- ============================================================================
...
-- ============================================================================
-- SECTION 21: RECOMMENDATIONS FOR PARALLEL QUERY ISSUES
-- ============================================================================

SELECT '=== DIAGNOSTIC SUMMARY & RECOMMENDATIONS ===' AS section;
SELECT 'Check the following for parallel query contention:' AS recommendation
UNION ALL
SELECT '1. Look at data_locks for GAP/NEXT-KEY locks that span wide ranges'
UNION ALL
SELECT '2. Check if isolation level is REPEATABLE-READ (causes phantom read protection via gap locks)'
UNION ALL
SELECT '3. Consider READ-COMMITTED isolation if gap locks are the issue'
UNION ALL
SELECT '4. Check innodb_autoinc_lock_mode=2 for parallel inserts'
UNION ALL
SELECT '5. Look for full table scans in statements (causes table-level locks)'
UNION ALL
SELECT '6. Check for missing indexes causing row-level locks to escalate'
UNION ALL
SELECT '7. Look at metadata_locks for DDL/DML conflicts'
UNION ALL
SELECT '8. Check redo log waits for high-throughput write contention'
UNION ALL
SELECT '9. Monitor adaptive hash index contention (disable if too much latch contention)'
UNION ALL
SELECT '10. Check if innodb_thread_concurrency is limiting parallel execution';

-- ============================================================================
-- END OF DIAGNOSTIC SCRIPT
-- ============================================================================
SELECT '=== DIAGNOSTIC COMPLETE ===' AS section, NOW() AS completed_at;
